...
Hexes of a colour represent a single species. Not much has happened by generation 1000, but you should see a first speciation event. Note the slight protraction between the actual branching time and the time speciation is detected.
...
The simulation can take quite long, but interesting things happen after many generations. Below is an example with $\sigma_z = 0.1$ & $\sigma_K = 1$.
...
`comrad` uses a dedicated data structure to encode these diversity-dependent models, a list containing (1) the name of the model, (2) the speciation function, (3) the extinction function, (4) a list of constrains for the initial parameter values, (5) a function used internally to check that the correct parameters are supplied, and (6) the name of the DD model in package `DDD`.
...
Note that this is different from the nomenclature used in package `DDD`, where the DD models are specified by a number. For conveniennce, I have included a function to quickly retrieve the `DDD` code corresponding to a given DD model:
...
Second, the optimisation needs to start from a set of initial values.
