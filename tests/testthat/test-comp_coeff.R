...
test_that("use", {
  ...
  # below: tests turned off bc sigma_comp = 0 is not accepted
  # expect_equal(comp_coeff(8, 8.1, 0), 0) # close values but no comp
  # expect_equal(comp_coeff(-0.3, -0.3, 0), 0) # same values but no comp (NaN!)
  expect_equal(comp_coeff(-10e4, 10e4, Inf), 1) # intense comp
  expect_equal(comp_coeff(5, -1, 0.2), comp_coeff(-5, 1, 0.2)) # symmetry
  expect_equal(comp_coeff(5, -1, 0.2), comp_coeff(-1, 5, 0.2)) # symmetry
})

test_that("abuse", {
  expect_is(comp_coeff(0.5, 0.6, 0.2), "numeric")
  expect_error(
  ...
  expect_error(
    ...
    regexp = "'sigma_comp' must be a numeric different from 0"
  )
  expect_error(
