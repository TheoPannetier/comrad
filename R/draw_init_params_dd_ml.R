...
draw_init_params_dd_ml <- function(nb_sets, phylos, dd_model) {
  ...
  alphas <- ifelse(with_alpha, stats::runif(nb_sets, 0, 1), rep(0, nb_sets))
  # ensure k prime is not smaller than n_max
  min_ks <- n_max * (lambdas - mus) / (1 - alphas) / lambdas
  ks <- pmax(min_ks, ceiling(n_max * 5 ^ stats::runif(nb_sets, -1, 1)))
